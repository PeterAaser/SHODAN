...
* Doing
  SBS og stimulus.
  sjekk hva som skjer hvis MEAME for en reg write req større enn buf size
...
* TODO Stim
** TODO Rewrite stimulus. Possibly from SHODAN?
** TODO SHODAN stim requests for single stim site
** TODO Multiple stim sites
...
*** Return of the unimplemented method
    Implemented a new pipe for ahh fuck it

** Round robin acting up again?
   Check if simply using the head instead of round robin works out
   
** Only head of list of stream is emitting
   Typically falls through the cracks. Try join vectorize instead of roundRobin or similar.
