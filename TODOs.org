...
* Learning
** TODO Sett deg inn i scalatags
** TODO Sbt mode i spacemacs
   
* Neste gang
** TODO Lage ferdig default setup i calls.scala.
** TODO Endre logging til å bare lese default config etter commit
  
* Doing
  Figure out why triggers aren't fired off correctly.
  
* Now 
** TODO Add DSP memory browser to frontend
*** TODO Clean up dsp routines. Remove write stuff.
*** TODO Set up RPC interfaces
*** TODO Render field with HTML
*** TODO Fetch data from DSP 
*** TODO Match selected data with DSP value
*** DONE Write a shitty version
*** TODO Evaluate the need for a proper browser
...
* Maybe
** TODO Optimize segment translator
...
* Udash
  kjør i scalafiddle for 60 checkboxes
  #+begin_src scala
    import io.udash._
    import scalatags.JsDom.all._

    val options = Seq.tabulate(60)(idx => idx.toString)
    val selected = SeqProperty[String]("39")

    println(div(
              CheckButtons.apply(selected, options, decorator = els => {
                                   div(els.map { case (in, name) => span(in, " ", name, " | ") })
                                 }),
              bind(selected)
            ))
  #+end_src
...
* How to run it all
  Når SHODAN startes så kan funksjonalitet kun nåes vha direkte URL (lol)
