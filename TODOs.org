* Neste gang
  se BitDrawing.dothing i launcher
  
  word -> List[field] -> String
  
  RegistryGroup burde ha en [Address, Word] -> Show
  render for hver register
  
  For å teste at det virker kjør alle write ops, coalesce og redraw.
...
** DONE Read and write bit fields
** DONE Coalesce reads and writes
** TODO Refactor HttpClient stuff, get the forest under control
** TODO Refine register representation.
