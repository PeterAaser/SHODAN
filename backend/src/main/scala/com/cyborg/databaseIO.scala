...
object DatabaseIO {
  ...
  def databaseStream[F[_]: Async](fcs: Stream[F,List[Stream[F,Array[Int]]]]): Stream[F,Byte] = fcs flatMap (channelStreamList =>
    {
      val unpacked: List[Stream[F, Vector[Byte]]] =
        channelStreamList.map(_.through(arrayBreaker(512)).through(utilz.vectorize(1024*4)))

      val spliced: Stream[F, Vector[Byte]] =
        (Stream[F, Vector[Byte]](Vector[Byte]()).repeat /: unpacked){
          (b: Stream[F, Vector[Byte]], a: Stream[F, Vector[Byte]]) => b.zipWith(a){
            (λ, µ) => µ ++: λ
          }
        }

      val rechunked: Stream[F, Byte] =
